...
void loop() {
  ...
  // 2. Baca Sensor Suhu (MODE SIMULASI VIDEO)
  byte temperature = 0;
  byte humidity = 0;
  dht11.read(&temperature, &humidity, NULL); // Tetap panggil biar nggak ada error log

  // --- MULAI MANIPULASI SUHU ---
  static float tempC = 22.0; // Mulai dari suhu dingin
  static float step = 1.0;   // Naik/turun 1 derajat per detik
  
  tempC += step;
  if (tempC >= 32.0) {
    step = -1.0; // Kalau udah panas, otomatis putar balik jadi turun
  } else if (tempC <= 22.0) {
    step = 1.0;  // Kalau udah dingin, otomatis naik lagi
  }
  // --- AKHIR MANIPULASI SUHU ---
